...
M4 is intentionally unbound to an execution engine because that historical plan predates the UI; select a new verified finished-UI build. Its slots require a new verified finished-UI build before execution. Alternate M4 walkthroughs explicitly refer to saved M2 runs and cannot count as fresh inference. M2 executions cannot be relabeled finished-UI rehearsals.
...
For paid campaign authority, actual analyst observations, owner acceptance of each rubric, refreshed prices/bounds, verified execution source and explicit spending authorization remain required by the unchanged runtime. The owner waived analyst feedback as a prerequisite for the local scripted demo, so technical review and UI implementation can proceed. A plan prepared from a dirty candidate is a review artifact; regenerate and verify it at the selected execution commit. Future UI or fixture changes require new bindings. No historical live store was opened or grant reused in this increment.
...
September 8: see [completed technical review](SECOPS-TECHNICAL-REVIEW-20260908.md) and [implemented local workspace](SECOPS-ANALYST-WORKSPACE.md). The scripted findings fail semantic review; no human case acceptance or live campaign completion is claimed.
