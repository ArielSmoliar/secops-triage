...
C. **Action (reversible):** Named-fixture preparation and a bound reviewer-mediated evaluation record are implemented in docs/SECOPS-EVALUATION-READINESS.md. Complete actual claim reviews and the remaining case set before the M2 campaign. Score raw model recommendation, each material claim, claim-to-citation support, omissions, deterministic outcome, reconciliation disagreement and final packet separately. Keep expected answers outside agent-visible input.
