...
C. **Action (reversible):** Named-fixture preparation and a bound reviewer-mediated evaluation record are implemented in docs/SECOPS-EVALUATION-READINESS.md. Nine draft cases now exist in docs/SECOPS-CASE-MATRIX-AND-CAMPAIGN.md. Complete owner case acceptance and actual claim reviews before the M2 campaign. Score raw model recommendation, each material claim, claim-to-citation support, omissions, deterministic outcome, reconciliation disagreement and final packet separately. Keep expected answers outside agent-visible input.
